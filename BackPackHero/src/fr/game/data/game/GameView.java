...
public record GameView(GameData data, ImageLoader loader) {
	...
	/**
	 * Draws the end menu on the application interface with the specified application context, width, height, and game data.
	 *
	 * @param context  The application context.
	 * @param width    The width of the menu.
	 * @param height   The height of the menu.
	 * @param data     The game data.
	 */
	public void drawEndMenu(ApplicationContext context, float width, float height, GameData data) {
		context.renderFrame(graphics -> {
			drawImage(graphics, loader.image("end.png"), 0,0, height, width);
		});
	}
