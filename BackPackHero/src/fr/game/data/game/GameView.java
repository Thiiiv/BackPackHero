...
public record GameView(GameData data, ImageLoader loader) {
	...
	public void drawRoom(Graphics2D graphics, float dimX, float dimY, float posX, float posY, Room room) {
	...
	public void drawMap(ApplicationContext context, int height, int width, GameData data) {
	...
	public void drawMonster(Graphics2D graphics, int height, int width, Monster monster) {
		...
		graphics.setColor(new Color(168, 38, 52, 255));
		graphics.fill(new RoundRectangle2D.Float((float) healthBar.getX(), (float) healthBar.getY(),
				(float) healthBar.getWidth() * ((float) monster.health() / monster.maxHealth()),
				(float) healthBar.getHeight(), 10, 10));
		graphics.setColor(Color.WHITE);
		Font font = new Font("Lucida Sans", Font.BOLD, 25);
		graphics.setFont(font);
		graphics.drawString(monster.health() + "/" + monster.maxHealth(), (posX + dimx) - font.getSize() * 3,
				posY + dimy + dimx / 8);
	}
	...
	public void draw(Graphics2D graphics, ApplicationContext context, GameData data) {
		...
		if (data.getMenuState() == true) {
			...
			drawMonster(graphics, (int) width, (int) height, new Monster());
			if (!data.getMapButtonState()) {
